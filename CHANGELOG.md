...
### Fixed / 修复
- **settings.json 热重载此前只存在于文档**：README 与功能表宣称“每 0.5s 热重载外部编辑”，但从未实现（`load()` 仅在启动时调用）。现在 0.5s 房管检查会比对磁盘内容与当前模型，不一致即应用——外部编辑调参无需重启；面板驱动的防抖写入窗口自动跳过，不会回跳。清理遗留的 `regen-app-icon.sh`（已被 `build-icon.sh` 取代）。

## [0.3.3] - 2026-09-17

### 新增 / Added
- **强制置顶（实验性）**：新增面板开关，参考 Lakr233/SkyLightWindow 的思路（私有 SkyLight 框架：创建"锁屏级"专属 Space 并将覆盖层窗口移入）。开启后特效显示在一切元素之上——Dock、菜单栏、右键菜单、第三方启动器，甚至锁屏。关闭时通过整体重建覆盖层窗口回到常规层级；显示器热插拔重建的新窗口自动补委托。私有 API 均有守卫，不可用时保持普通层级。默认关闭。

### Fixed / 修复
- **「恢复默认设置」保留点击统计**：计数是统计资料而非设置参数，重置参数不再清零。

### 优化 / Performance
- DMG 安装窗口图标位置微调（y=145），背景生成脚本 `tools/make-dmg-background.sh` 入库。
